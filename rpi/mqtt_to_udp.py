...
udp_sock.bind(("0.0.0.0", RECEIVER_PORT))
print(f"Listning on port {RECEIVER_PORT}")
...
hello_ack_packet = bytes([1]) + b"HELLO"
...
def send_hello_message():
    while  udp_host == ' ':
        discovery_sock.sendto(hello_packet, ("255.255.255.255", DISCOVERY_PORT))
        time.sleep(1)

threading.Thread(target=send_hello_message, daemon=True).start()
"""

def discovery_loop():
    global udp_host
    while True:
        data, addr = discovery_sock.recvfrom(64)
        udp_host = addr[0]
        discovered_event.set()
        discovery_sock.sendto(hello_ack_packet, (udp_host, DISCOVERY_PORT))
...
def send_raw_hid(modifier, keycode):
    ...
    if not discovered_event.is_set():
        print("Pico not discovered yet - dropping packet")
        return False
    ...
    packet = bytes([2, 0, seq_counter, modifier, keycode])
    ...
    while retries > 0:
        udp_sock.sendto(packet, (udp_host, SEND_PORT))
